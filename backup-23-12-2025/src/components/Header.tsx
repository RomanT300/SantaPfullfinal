...
const navItems: NavItem[] = [
  ...
  { to: '/dashboard', label: 'Analíticas', icon: <LayoutDashboard size={16} /> },
  {
    ...
    icon: <Wrench size={16} />,
    children: [
      { to: '/maintenance', label: 'Biosems', icon: <Wrench size={16} /> },
      { to: '/tropack-maintenance', label: 'Plan Tropack', icon: <Factory size={16} /> },
              ...
              item.children ? (
                ...
                <div key={item.to} className="relative" ref={dropdownRef}>
                  ...
                  >
                    {item.icon}
                    {item.label}
                    <ChevronDown size={14} className={`transition-transform ${openDropdown === item.to ? 'rotate-180' : ''}`} />
                  </button>
                  {openDropdown === item.to && (
                    <div className="absolute top-full left-0 mt-1 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50">
                ...
                item.children ? (
                  ...
                  <div key={item.to}>
                    ...
                    >
                      ...
                      <ChevronDown size={16} className={`transition-transform ${openDropdown === item.to ? 'rotate-180' : ''}`} />
                    </button>
                    {openDropdown === item.to && (
                      <div className="ml-4 mt-1 flex flex-col gap-1 border-l-2 border-gray-200 dark:border-gray-700 pl-4">
                        {item.children.map((child) => (
                          <NavLink
                            key={child.to}
                            to={child.to}
