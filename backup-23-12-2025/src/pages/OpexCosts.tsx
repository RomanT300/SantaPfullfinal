...
import { DollarSign, TrendingUp, TrendingDown, AlertCircle, Upload, Download, Droplets, Users, Wrench, Zap, FlaskConical, ArrowUpRight, ArrowDownRight, Table, BarChart3, Save, Check, Loader2, FileText, FileSpreadsheet, Pencil } from 'lucide-react'
...
const EDITABLE_FIELDS = [
  { key: 'volume_m3', label: 'Volumen (m³)', prefix: '', suffix: ' m³' },
  { key: 'cost_agua', label: 'Agua', prefix: '$', suffix: '' },
  { key: 'cost_personal', label: 'Personal', prefix: '$', suffix: '' },
  { key: 'cost_mantenimiento', label: 'Mantenimiento', prefix: '$', suffix: '' },
  { key: 'cost_energia', label: 'Energía', prefix: '$', suffix: '' },
  { key: 'cost_floculante', label: 'Floculante', prefix: '$', suffix: '' },
  { key: 'cost_coagulante', label: 'Coagulante', prefix: '$', suffix: '' },
  { key: 'cost_estabilizador_ph', label: 'Estab. pH', prefix: '$', suffix: '' },
  { key: 'cost_dap', label: 'DAP', prefix: '$', suffix: '' },
  { key: 'cost_urea', label: 'Urea', prefix: '$', suffix: '' },
  { key: 'cost_melaza', label: 'Melaza', prefix: '$', suffix: '' },
] as const
...
export default function OpexCosts() {
  ...
  const [pendingChanges, setPendingChanges] = useState<Map<string, Partial<OpexCost>>>(new Map())
  ...
  const saveEdit = useCallback(async (id: string, field: EditableField, value: string) => {
    ...
    const record = allPlantsData.find(r => r.id === id)
    if (!record) return
    ...
    try {
      const response = await fetch(`/api/opex/${id}`, {
        method: 'PUT',
        credentials: 'include',
  ...
  const handleKeyDown = (e: React.KeyboardEvent, id: string, field: EditableField) => {
    if (e.key === 'Enter') {
      e.preventDefault()
  ...
  const downloadCSV = () => {
    if (allPlantsData.length === 0) return
    ...
    const rows = allPlantsData.map(record => {
      const totalCost =
        record.cost_agua + record.cost_personal + record.cost_mantenimiento +
        record.cost_energia + record.cost_floculante + record.cost_coagulante +
        record.cost_estabilizador_ph + record.cost_dap + record.cost_urea + record.cost_melaza

      const costPerM3 = record.volume_m3 > 0 ? totalCost / record.volume_m3 : 0

      return [
        record.id,
        plantMap.get(record.plant_id) || record.plant_id,
        record.period_date,
        record.volume_m3,
        record.cost_agua,
        record.cost_personal,
        record.cost_mantenimiento,
        record.cost_energia,
        record.cost_floculante,
        record.cost_coagulante,
        record.cost_estabilizador_ph,
        record.cost_dap,
        record.cost_urea,
        record.cost_melaza,
        record.notes || '',
    ...
    link.download = `opex_${selectedYear}_${new Date().toISOString().slice(0, 10)}.csv`
    link.click()
    URL.revokeObjectURL(link.href)
  }

  // Download PDF report using print
  const downloadPDF = () => {
    // Create a printable version of the charts
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Download buttons - available for all users when there's data */}
            {allPlantsData.length > 0 && (
              <>
                <button
                  onClick={downloadCSV}
                  className="px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/30 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2 font-medium"
                >
                ...
                <button
                  onClick={downloadPDF}
                  className="px-4 py-2.5 rounded-xl bg-rose-500 hover:bg-rose-600 text-white shadow-lg shadow-rose-500/30 hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2 font-medium"
                >
                  <FileText size={18} />
                  Descargar Informe
                </button>
        ...
        {allPlantsData.length > 0 && (
          <div className="flex gap-2 mb-6 no-print">
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-medium flex items-center gap-2 transition-all ${
                activeTab === 'charts'
                  ...
                  : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 shadow-lg shadow-gray-200/50 dark:shadow-none hover:shadow-xl'
              }`}
            >
              <BarChart3 size={18} />
              Graficas
            ...
            >
              <Table size={18} />
              Datos
              {isAdmin && <Pencil size={14} className="opacity-70" />}
            </button>
          </div>
        )}
        ...
        {loadingAllPlants ? (
          ...
          <div id="print-content" ref={printRef}>
            ...
            {/* Data Tab Content - Editable Table */}
            {activeTab === 'data' && (
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                      Datos OPEX - {selectedYear}
                      {isAdmin && (
                        <span className="px-2 py-1 text-xs bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full flex items-center gap-1">
                          <Pencil size={12} />
                          Editable
                        </span>
                      )}
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {allPlantsData.length} registros | Haga clic en una celda para editar
                    </p>
                  </div>
                  {savingIds.size > 0 && (
                  ...
                  <table className="min-w-full">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-gray-50 dark:bg-gray-900/80">
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider sticky left-0 bg-gray-50 dark:bg-gray-900/80">Planta</th>
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Fecha</th>
                        {EDITABLE_FIELDS.map(field => (
                          <th key={field.key} className="px-3 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider whitespace-nowrap">
                            {field.label}
                          </th>
                        ))}
                        <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Total</th>
                        <th className="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">$/m³</th>
                        <th className="px-4 py-3 text-center text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Estado</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                      {allPlantsData
                        .sort((a, b) => {
                          const plantCompare = getPlantName(a.plant_id).localeCompare(getPlantName(b.plant_id))
                        ...
                        .map((record) => {
                          ...
                          return (
                            <tr key={record.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                              <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white sticky left-0 bg-white dark:bg-gray-800">
                                {getPlantName(record.plant_id)}
                              </td>
                              <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300 whitespace-nowrap">
                                {new Date(record.period_date).toLocaleDateString('es-EC', { year: 'numeric', month: 'short' })}
                              </td>
                              {EDITABLE_FIELDS.map(field => {
                                const value = record[field.key as keyof OpexCost] as number
                                const isEditing = editingCell?.id === record.id && editingCell?.field === field.key
                                  ...
                                  <td key={field.key} className="px-3 py-2 text-right">
                                    {isEditing && isAdmin ? (
                                      <input
                                        ...
                                        onBlur={() => saveEdit(record.id, field.key, editValue)}
                                        onKeyDown={(e) => handleKeyDown(e, record.id, field.key)}
                                        className="w-24 px-2 py-1 text-right text-sm border border-indigo-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                                        autoFocus
                                      />
                                    ) : (
                                      <button
                                        onClick={() => isAdmin && startEditing(record.id, field.key, value)}
                                        ...
                                        disabled={!isAdmin}
                                      >
                                        {field.prefix}{value.toLocaleString('es-EC', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}{field.suffix}
                                      </button>
                                    )}
                                  </td>
                                )
                              ...
                              <td className="px-4 py-3 text-right">
                                <span className={`text-sm font-bold ${
                                  costPerM3 < 1 ? 'text-emerald-600 dark:text-emerald-400' :
                                  costPerM3 > 3 ? 'text-rose-600 dark:text-rose-400' :
                                  'text-gray-900 dark:text-white'
                                }`}>
                                  ${costPerM3.toFixed(4)}
                                </span>
                              ...
                              <td className="px-4 py-3 text-center">
                                {isSaving ? (
                                  ...
                                  <span className="w-4 h-4 block mx-auto" />
                                )}
                              </td>
                            </tr>
                          )
                ...
                {isAdmin && (
                  <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700">
                    <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <Pencil size={14} />
                      <span>Haga clic en cualquier valor numerico para editarlo. Los cambios se guardan automaticamente.</span>
                    </div>
                  </div>
                )}
                ...
                {/* Info for non-admin */}
                {!isAdmin && (
                  <div className="px-6 py-4 bg-indigo-50 dark:bg-indigo-900/20 border-t border-indigo-200 dark:border-indigo-800">
                    <div className="flex items-center gap-2 text-sm text-indigo-700 dark:text-indigo-300">
                      <AlertCircle size={14} />
                      <span>Solo los administradores pueden editar los datos de costos OPEX.</span>
                    </div>
                  </div>
                )}
              </div>
