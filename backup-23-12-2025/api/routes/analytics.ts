...
router.post(
  ...
  [
    ...
    body('stream').optional().isIn(['influent', 'reactor', 'effluent']),
  ],
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        // Mapear tipo entrada/reactor/salida a influent/reactor/effluent
        if (tipo === 'entrada' || tipo === 'afluente') tipo = 'influent'
        if (tipo === 'reactor' || tipo === 'biorreactor') tipo = 'reactor'
        if (tipo === 'salida' || tipo === 'efluente') tipo = 'effluent'
        ...
        if (!['influent', 'reactor', 'effluent'].includes(tipo)) {
          errors.push(`Línea ${i + 1}: Tipo "${values[tipoIdx]}" no válido (usar entrada/reactor/salida o influent/reactor/effluent)`)
          continue
        }
