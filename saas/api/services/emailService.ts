...
// Email templates
type EmailTemplate = 'maintenance_reminder' | 'parameter_alert' | 'emergency_alert' | 'task_assignment' | 'task_reminder' | 'ticket_new' | 'invitation' | 'temp_password'
...
type TemplateData = MaintenanceReminderData | ParameterAlertData | EmergencyAlertData | TaskAssignmentData | TaskReminderData | TicketNewData | InvitationData | TempPasswordData
...
class EmailService {
  ...
  private getTemplate(template: EmailTemplate, data: TemplateData): { subject: string; html: string } {
    switch (template) {
      ...
      case 'invitation': {
        const d = data as InvitationData
        ...
        return {
          subject: `📩 Invitación para unirse a ${d.organizationName} - PTAR SaaS`,
          html: `
            <!DOCTYPE html>
            <html>
            <head>
              <style>
                ...
                .invite-box { background: white; border: 2px solid #6366f1; padding: 25px; margin: 20px 0; border-radius: 10px; text-align: center; }
                .role-badge { display: inline-block; background: #6366f1; color: white; padding: 5px 15px; border-radius: 20px; font-weight: bold; }
                .footer { background: #1e293b; color: #94a3b8; padding: 15px; text-align: center; border-radius: 0 0 10px 10px; font-size: 12px; }
                .btn { display: inline-block; background: #6366f1; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; margin-top: 20px; font-weight: bold; font-size: 16px; }
                .btn:hover { background: #4f46e5; }
                .expire-note { color: #64748b; font-size: 12px; margin-top: 15px; }
              </style>
            </head>
            <body>
              <div class="container">
                ...
                <div class="content">
                  <p>Hola${d.inviteeName ? ` ${d.inviteeName}` : ''},</p>

                  <p><strong>${d.invitedBy}</strong> te ha invitado a unirte a la plataforma PTAR SaaS para gestionar las plantas de tratamiento.</p>

                  <div class="invite-box">
                    <h2 style="margin: 0 0 10px 0; color: #1e293b;">${d.organizationName}</h2>
                    <p style="margin: 0 0 15px 0; color: #64748b;">Tu rol asignado:</p>
                    <span class="role-badge">${roleLabels[d.role] || d.role}</span>

                    <a href="${d.inviteUrl}" class="btn">Aceptar Invitación</a>

                    <p class="expire-note">Este enlace expira el ${new Date(d.expiresAt).toLocaleDateString('es-EC', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                  </div>

                  <p>Si no esperabas esta invitación, puedes ignorar este mensaje.</p>
                </div>
                <div class="footer">
                  PTAR SaaS | Gestión de Plantas de Tratamiento de Agua
                </div>
              </div>
            </body>
            </html>
      ...
      case 'temp_password': {
        const d = data as TempPasswordData
        return {
          subject: `🔐 Tu contraseña temporal - ${d.organizationName}`,
          html: `
            <!DOCTYPE html>
            <html>
            <head>
              <style>
                ...
                .content { background: #f8fafc; padding: 30px; border: 1px solid #e2e8f0; }
                .password-box { background: #fef3c7; border: 2px solid #f59e0b; padding: 25px; margin: 20px 0; border-radius: 10px; text-align: center; }
                .password { font-family: monospace; font-size: 28px; font-weight: bold; color: #92400e; letter-spacing: 3px; padding: 15px; background: white; border-radius: 8px; display: inline-block; }
                .warning { background: #fef2f2; border-left: 4px solid #dc2626; padding: 15px; margin: 20px 0; }
                .footer { background: #1e293b; color: #94a3b8; padding: 15px; text-align: center; border-radius: 0 0 10px 10px; font-size: 12px; }
                .btn { display: inline-block; background: #f59e0b; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; margin-top: 20px; font-weight: bold; }
              </style>
            </head>
            <body>
              <div class="container">
                ...
                <div class="content">
                  ...
                  <p>Se ha generado una contraseña temporal para tu cuenta en <strong>${d.organizationName}</strong>.</p>

                  <div class="password-box">
                    <p style="margin: 0 0 10px 0; color: #92400e;">Tu contraseña temporal es:</p>
                    <div class="password">${d.tempPassword}</div>
                  </div>

                  <div class="warning">
                    <strong>⚠️ Importante:</strong>
                    <ul style="margin: 10px 0 0 0; padding-left: 20px;">
                      <li>Esta contraseña es de un solo uso</li>
                      <li>Deberás cambiarla inmediatamente después de iniciar sesión</li>
                      <li>No compartas esta contraseña con nadie</li>
                    </ul>
                  </div>

                  <div style="text-align: center;">
                    <a href="${d.loginUrl}" class="btn">Iniciar Sesión</a>
                  </div>
                </div>
                <div class="footer">
                  PTAR SaaS | Gestión de Plantas de Tratamiento de Agua
  ...
  async sendInvitation(to: string, data: InvitationData): Promise<boolean> {
    return this.sendEmail(to, 'invitation', data)
  }

  async sendTempPassword(to: string, data: TempPasswordData): Promise<boolean> {
    return this.sendEmail(to, 'temp_password', data)
  }
}

// Singleton instance
export const emailService = new EmailService()
export type { MaintenanceReminderData, ParameterAlertData, EmergencyAlertData, TaskAssignmentData, TaskReminderData, InvitationData, TempPasswordData }
