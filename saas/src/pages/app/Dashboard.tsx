    ...
    Promise.all(fetchList.map(item => fetchParam(item.param, item.stream)))
      .then(results => {
        ...
        if (!isAllPlants) {
          ...
          // All plants: show real values per plant (no averaging)
          // Structure: { param_stream_plantName: { date: value } }
          const seriesByPlant: Record<string, Record<string, number>> = {}
          const allValsForKpis: Record<string, number[]> = {}

          // Process each selected parameter - collect values per plant
          selectedParams.forEach(param => {
            allValsForKpis[param] = []
              ...
              if (streamFilter === 'all' || streamFilter === 'influent') {
                ...
                if (influentResult && influentResult.data.length) {
                  influentResult.data.forEach((row: any) => {
                    const plantName = plants.find(p => p.id === row.plant_id)?.name || row.plant_id || 'Unknown'
                    const key = `${param}_influent_${plantName}`
                    if (!seriesByPlant[key]) seriesByPlant[key] = {}
                    seriesByPlant[key][row.measurement_date] = row.value
                    allValsForKpis[param].push(row.value)
                  })
                }
              }
              // Reactor
              if (streamFilter === 'all' || streamFilter === 'reactor') {
                ...
                if (reactorResult && reactorResult.data.length) {
                  reactorResult.data.forEach((row: any) => {
                    const plantName = plants.find(p => p.id === row.plant_id)?.name || row.plant_id || 'Unknown'
                    const key = `${param}_reactor_${plantName}`
                    if (!seriesByPlant[key]) seriesByPlant[key] = {}
                    seriesByPlant[key][row.measurement_date] = row.value
                    allValsForKpis[param].push(row.value)
                  })
              ...
              if (streamFilter === 'all' || streamFilter === 'effluent') {
                ...
                if (effluentResult && effluentResult.data.length) {
                  effluentResult.data.forEach((row: any) => {
                    const plantName = plants.find(p => p.id === row.plant_id)?.name || row.plant_id || 'Unknown'
                    const key = `${param}_effluent_${plantName}`
                    if (!seriesByPlant[key]) seriesByPlant[key] = {}
                    seriesByPlant[key][row.measurement_date] = row.value
                    allValsForKpis[param].push(row.value)
                  })
          ...
          // Generate demo data if empty - one series per plant
          const hasData = Object.keys(seriesByPlant).length > 0
          if (!hasData) {
            selectedParams.forEach(param => {
              plants.forEach(pl => {
                ...
                if (flowMode === 'split') {
                  // Influent demo data per plant
                  if (streamFilter === 'all' || streamFilter === 'influent') {
                    const key = `${param}_influent_${pl.name}`
                    seriesByPlant[key] = {}
                    dates.forEach((d, i) => {
                      const season = Math.sin((i / dates.length) * Math.PI * 2)
                      const base = param === 'DQO' ? 1000 : param === 'SS' ? (bases.SS || 100) : (bases.pH || 7.3)
                      const jitter = param === 'pH' ? (Math.random() * 0.3 - 0.15) : (Math.random() * 40 - 20)
                      const val = param === 'pH' ? (base + season * 0.15 + jitter) : (base + season * 50 + jitter)
                      seriesByPlant[key][d] = Math.max(0, Math.round(val * 100) / 100)
                      allValsForKpis[param] = allValsForKpis[param] || []
                      allValsForKpis[param].push(seriesByPlant[key][d])
                    })
                  }
                  // Reactor demo data per plant
                  if (streamFilter === 'all' || streamFilter === 'reactor') {
                    const key = `${param}_reactor_${pl.name}`
                    seriesByPlant[key] = {}
                    dates.forEach((d, i) => {
                      const season = Math.sin((i / dates.length) * Math.PI * 2)
                      const base = param === 'DQO' ? 500 : param === 'SS' ? (bases.SS ? bases.SS * 0.7 : 70) : (bases.pH || 7.3)
                  ...
                  // Effluent demo data per plant
                  if (streamFilter === 'all' || streamFilter === 'effluent') {
                    const key = `${param}_effluent_${pl.name}`
                    seriesByPlant[key] = {}
                    dates.forEach((d, i) => {
                      const season = Math.sin((i / dates.length) * Math.PI * 2)
                      const base = param === 'DQO' ? 180 : param === 'SS' ? (bases.SS ? Math.max(30, bases.SS * 0.5) : 40) : (bases.pH || 7.3)
          ...
          // Merge: real values per plant (no averaging)
          merged = dates.map(d => {
            const obj: any = { measurement_date: d }
            Object.keys(seriesByPlant).forEach(key => {
              obj[key] = seriesByPlant[key][d] ?? null
            })
            return obj
          })

          // KPIs for all plants mode - stats from all real values
          selectedParams.forEach(param => {
            const vals = allValsForKpis[param] || []
            const count = vals.length
            const sum = vals.reduce((a, b) => a + b, 0)
            const min = vals.length ? Math.min(...vals) : 0
            const max = vals.length ? Math.max(...vals) : 0
        ...
        <div id="chart-container" className="h-96 bg-white dark:bg-gray-800 rounded shadow p-2">
          <ResponsiveContainer>
            <ComposedChart data={data}>
              ...
              {plantId === '' ? (
                // All plants mode: show real values per plant (dynamic lines)
                <>
                  {(() => {
                    // Get all data keys dynamically (exclude measurement_date)
                    const dataKeys = data.length > 0 ? Object.keys(data[0]).filter(k => k !== 'measurement_date') : []
                    ...
                    const defaultColors = ['#06b6d4', '#ec4899', '#84cc16', '#f97316', '#6366f1']
                    let colorIndex = 0

                    return dataKeys.map(key => {
                      // Parse key format: param_stream_plantName or param_plantName
                      const parts = key.split('_')
                      const param = parts[0]
                      let stream: string | null = null
                      let plantName: string

                      if (flowMode === 'split' && parts.length >= 3) {
                        stream = parts[1] // influent, reactor, effluent
                        plantName = parts.slice(2).join('_')
                      } else {
                        plantName = parts.slice(1).join('_')
                      }

                      // Get color based on plant
                      const color = plantColors[plantName] || defaultColors[colorIndex++ % defaultColors.length]

                      // Build display name
                      const streamLabel = stream === 'influent' ? ' (afluente)' : stream === 'reactor' ? ' (reactor)' : stream === 'effluent' ? ' (efluente)' : ''
                      const displayName = `${param} - ${plantName}${streamLabel}`

                      // Determine axis and styling based on param
                      const yAxisId = param === 'pH' ? 'right' : 'left'
                      const strokeDasharray = stream === 'reactor' ? '3 3' : stream === 'effluent' ? '5 5' : undefined
