...
export default function Admin() {
  ...
  const canDeletePlants = user?.role === 'admin' || user?.role === 'owner'
  ...
  useEffect(() => {
    ...
    load()
  }, [activeTab, isAdminOrOwner, analyticsFilter])
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <div className="text-sm mb-4">
        Rol actual: <span className="font-semibold">{user?.role || 'Desconocido'}</span>
        {user?.name && <span className="text-gray-500 ml-2">({user.name})</span>}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Pestaña de gestión solo Admin/Owner */}
        <div className="bg-white dark:bg-gray-800 rounded border p-4 md:col-span-2">
          ...
          {!isAdminOrOwner ? (
            <div className="text-sm text-gray-600">Esta sección requiere rol <span className="font-semibold">admin</span> u <span className="font-semibold">owner</span>. Inicia sesión con un usuario con permisos adecuados.</div>
          ) : (
