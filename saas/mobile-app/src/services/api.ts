...
// Configure base URL via environment variable or fallback
// For React Native/Expo, use process.env or Constants.expoConfig
const getApiBaseUrl = () => {
  // Check if running in Expo with config
  try {
  ...
  // Fallback to dev/prod detection
  if (typeof __DEV__ !== 'undefined' && __DEV__) {
    return 'http://localhost:8080/api' // Change to your server IP for mobile testing
  }

  return 'https://api.yourcompany.com/api' // Production URL - configure per deployment
}
...
export const login = async (email: string, password: string): Promise<{ user: User; token: string }> => {
  ...
  const fullUser: User = {
    ...
    organizationSlug: user.organizationSlug || organization?.slug
  }
