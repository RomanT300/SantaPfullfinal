...
// Admins see all plants; supervisors/operators see only their assigned plant
router.get('/', requireAuth, async (req: Request, res: Response) => {
  const { search, status } = (req.query || {}) as Record<string, string>
  const user = (req as any).user

  try {
    let data = plantsDAL.getAll({ search, status })

    // Filter by plantId for non-admin users
    if (user && user.plantId && user.role !== 'admin') {
      data = data.filter((plant: any) => plant.id === user.plantId)
    }
