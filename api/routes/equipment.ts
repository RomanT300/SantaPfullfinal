...
// DELETE /api/equipment/scheduled/reset/:plantId/:year - Delete all scheduled tasks for plant and year
router.delete('/scheduled/reset/:plantId/:year', requireAuth, async (req: Request, res: Response) => {
  const { plantId, year } = req.params

  try {
    const deleted = equipmentScheduledMaintenanceDAL.deleteByPlantAndYear(plantId, parseInt(year))
...
// PUT /api/equipment/scheduled/:id - Update scheduled task (change date)
router.put('/scheduled/:id', requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const updated = equipmentScheduledMaintenanceDAL.updateScheduledDate(id, scheduledDate)
    res.json({ success: true, data: updated })
  } catch (error: any) {
    res.status(400).json({ success: false, error: error.message })
  }
