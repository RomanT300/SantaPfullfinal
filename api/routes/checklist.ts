...
router.get('/today/:plantId', (req: Request, res: Response) => {
  try {
    ...
    if (!checklist) {
      ...
      // If no template specified, get ALL active templates for the plant
      if (templateItems.length === 0) {
        const templates = db.prepare(`
          SELECT id FROM checklist_templates WHERE plant_id = ? AND is_active = 1 ORDER BY template_code
  ...
  try {
    ...
    let query = `
      SELECT
        ...
        CASE
          -- SS Reactor and sludge tank - operational solids measurements
          WHEN (LOWER(dci.item_description) LIKE '%ss reactor%' OR LOWER(dci.item_description) LIKE '%ss%reactor%')
          THEN 'reactor_solids'
          WHEN (LOWER(dci.item_description) LIKE '%ss tanque%lodo%' OR LOWER(dci.item_description) LIKE '%tanque de lodos%')
          THEN 'sludge_solids'
          -- Purga minutes - operational runtime
          WHEN LOWER(dci.item_description) LIKE '%purga%'
            OR LOWER(dci.item_description) LIKE '%minutos de purga%'
          THEN 'purge'
          -- Energy
      ...
      -- But INCLUDE operational SS measurements (SS Reactor, SS Tanque Lodos) and purga
      AND NOT (
        (LOWER(dci.item_description) LIKE '%ph%' AND NOT LOWER(dci.item_description) LIKE '%biomasa%')
        OR LOWER(dci.item_description) LIKE '%dqo%'
        OR LOWER(dci.item_description) LIKE '%demanda química%'
        OR (LOWER(dci.item_description) LIKE '%sólidos suspendidos%' AND NOT (LOWER(dci.item_description) LIKE '%reactor%' OR LOWER(dci.item_description) LIKE '%lodo%'))
        OR (LOWER(dci.item_description) LIKE '%solidos suspendidos%' AND NOT (LOWER(dci.item_description) LIKE '%reactor%' OR LOWER(dci.item_description) LIKE '%lodo%'))
        OR (LOWER(dci.item_description) LIKE '%ss%' AND LOWER(dci.item_description) LIKE '%agua%' AND NOT LOWER(dci.item_description) LIKE '%reactor%' AND NOT LOWER(dci.item_description) LIKE '%lodo%')
    ...
    const categories = ['reactor_solids', 'sludge_solids', 'purge', 'energy', 'chemicals', 'pressure', 'flow', 'level', 'runtime', 'other']
