...
export default function PlantSelectScreen() {
  ...
  const loadPlants = async () => {
    try {
      ...
      const activePlants = data.filter(p => p.status === 'active')
      setPlants(activePlants)

      // If user has only one plant assigned (operator/supervisor), auto-select it
      if (activePlants.length === 1 && user?.plantId) {
        await selectPlant(activePlants[0])
      }
    } catch (error: any) {
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerSubtitle}>
            {user?.plantId
              ? 'Tu planta asignada'
              : 'Selecciona la planta a inspeccionar'}
          </Text>
        </View>
