...
// Configure base URL - dynamically detect or use tunnel URL
// For production, this will use the current host
const getApiBaseUrl = () => {
  // If running in browser, detect the host
  if (typeof window !== 'undefined' && window.location) {
    const host = window.location.hostname
    // If accessing via tunnel or production domain, use the backend tunnel
    if (host.includes('loca.lt') || host.includes('ngrok')) {
      // Use the backend tunnel URL
      return 'https://spicy-crews-pump.loca.lt/api'
    }
    if (host !== 'localhost') {
